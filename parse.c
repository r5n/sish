...
print_command(struct sish_command *comm)
{
    ...
    for (tmp = comm; tmp; tmp = tmp->next) {
        ...
        printf("\tOUT to %s", tmp->output);
        printf("\tIN from %s", tmp->input);
        if (tmp->append)
            printf("\tAPPEND to %s", tmp->output);	
        switch (tmp->conn) {
            case PIPE:
                printf("PIPE");
                break;
    ...
    for (i = 0; i < len; i++) {
        ...
        if (strpbrk(tokens[i], SPECIAL) != NULL) {
            ...
            switch (curr->conn) {
                case IN:
                    cmd = 0;
                    curr->input = tokens[i+1];
                    i++;
                    break;
                case OUT:
                    cmd = 0;
                    curr->output = tokens[i+1];
                    i++;
                    break;
                case APPEND:
                    cmd = 0;
